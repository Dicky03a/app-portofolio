...
const Certificates: React.FC = () => {
    const { certificates } = usePage<PageProps>().props;

    if (!certificates || certificates.length === 0) {
        return null; // optional: bisa ganti dengan empty state
    }
                    ...
                    {certificates.map((cert, index) => (
                        ...
                        >
                            ...
                            <div className="mt-auto flex items-center justify-between border-t border-white/5 pt-4">
                                ...
                                {cert.credential_url && <ExternalLink size={16} className="text-gray-500 transition-colors group-hover:text-white" />}
                            </div>
                        </motion.a>
                    ))}
                </div>
